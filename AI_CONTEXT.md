...
- **当前版本：v1.1.5（versionCode 67，构建时间 2026-08-05）**——版本更新规则：每次构建后 bump，安装包命名 `MeshChat-vX.Y.Z-debug.apk` 存于工程根目录
...
### 当前阻塞
- **⚠️ v1.1.3 二轮融合 + v1.1.4 删除对话修复 + v1.1.5 调试中心，待用户真机验证后提交推送**：改动全部在工作区未提交（v1.1.3 的 19 修改 + 8 新增，v1.1.4 的 6 文件小改，v1.1.5 的 DebugStats 内核/埋点/UI 约 12 文件）。**待办**：① 用户安装 `MeshChat-v1.1.5-debug.apk` 验证（删除对话后 Mesh 页节点消失、安全中心入口、对话归档/删除/未读、附近/历史分组、新图标、**调试中心各板块实时数据 + 7 项调节 + 暂停/清零**）；② 确认后 git add + commit + push origin/main。
...
### 下一步首要任务
0. **v1.1.5 真机验证（当前版本）**：用户安装 `MeshChat-v1.1.5-debug.apk` → ① 删除对话后 Mesh 页该节点消失（在线节点会重新被发现，属预期）② 安全中心入口/本地评估 ③ 对话归档/删除/未读/最后消息 ④ Mesh 页附近/历史分组 + 新图标 ⑤ **调试中心**：进 Profile→调试中心，观察五板块实时数据（收发包速率随心跳/消息跳动、BLE 统计、节点 RSSI、送达确认率）、调节 7 项（刷新间隔/窗口/单位/板块/排序/暂停/清零）生效 → 确认后提交推送 origin/main（v1.1.3 融合 + v1.1.4 修复 + v1.1.5 调试中心）。
