...
- **当前版本：v1.1.9（versionCode 71，构建时间 2026-08-05）**——版本更新规则：每次构建后 bump，安装包命名 `MeshChat-vX.Y.Z-debug.apk` 存于工程根目录
...
### 当前阻塞
- **⚠️ v1.1.3 二轮融合 + v1.1.4 删除对话修复 + v1.1.5 调试中心 + v1.1.6 FGS 权限 + v1.1.7 并发增强 + v1.1.8 崩溃修复（真机已确认正常）+ v1.1.9 调试中心主动控制，待用户真机验证后提交推送**：改动全部在工作区未提交（v1.1.3 的 19 修改 + 8 新增，v1.1.4 的 6 文件小改，v1.1.5 的 DebugStats 内核/埋点/UI 约 12 文件，v1.1.6 的 Manifest 权限 2 行，v1.1.7 的 ConcurrentHashMap 3 文件，v1.1.8 的 ViewModel 属性顺序 + try-catch，v1.1.9 的 DebugControl/控制面/UI 约 10 文件）。**待办**：① 用户安装 `MeshChat-v1.1.9-debug.apk` 验证（**重点：能正常打开 + 主动控制面板**：心跳 0.5s/1s/2s/5s 切换后收发包速率变化、暂停/恢复广播+扫描后对端发现状态变化、手动 PING 后统计即时 +1、恢复默认还原；其余：删除对话后 Mesh 页节点消失、安全中心、归档/删除/未读、附近/历史分组、新图标、调试中心观察板块）；② 确认后 git add + commit + push origin/main。
...
### 下一步首要任务
0. **v1.1.9 真机验证（当前版本）**：用户安装 `MeshChat-v1.1.9-debug.apk` → ① 能正常打开（v1.1.8 已确认）② **主动控制面板**（Profile→调试中心→主动控制）：心跳切 0.5s/2s/5s 后 FramesCard 的 PING 速率随之变化；「暂停广播+扫描」后对端"发现你"消失、恢复后重现；「发 PING」后统计 PING 发送 +1 且对端（若同装）收到；「恢复默认」还原 1s/3s 档位 ③ 其余回归：删除对话后节点消失、安全中心、归档/删除/未读、附近/历史分组、新图标、调试中心观察板块 → 确认后提交推送 origin/main（v1.1.3~v1.1.9 全部积压）。
