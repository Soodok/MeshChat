...
- **当前版本：v1.1.7（versionCode 69，构建时间 2026-08-05）**——版本更新规则：每次构建后 bump，安装包命名 `MeshChat-vX.Y.Z-debug.apk` 存于工程根目录
...
- **v1.1.7 启动崩溃真根因（2026-08-05，用户实测 v1.1.6 仍崩 + 确认 v1.1.3/v1.1.4 可打开）**：**FGS 权限不是主因**（v1.1.3/v1.1.4 无该权限也能打开），崩溃点在 v1.1.5 调试中心：`MeshChatViewModel.startDebugLoop()`（init 即启动、首轮不 delay）每 1s 在主线程调 `debugStats.snapshot()` → 触发 MeshService 注入的 `peersProvider()` **遍历 `peerEntries`（普通 LinkedHashMap）**；而 MeshService 后台协程（`Dispatchers.Default`）每 200ms tick/collect **并发写** `peerEntries`/`routeEntries` → **ConcurrentModificationException**；viewModelScope 协程异常未捕获 → 主线程 uncaughtExceptionHandler → **进程崩溃（屡次停止运行，界面都没进去）**。时序吻合：启动即撞（首轮 snapshot 无 delay + 200ms 心跳高频写入），每次必崩。**为何 v1.1.3/v1.1.4 正常**：无 startDebugLoop，主线程从不遍历这两个 LinkedHashMap。**为何单测全绿**：单测单线程驱动、无并发。修复三管齐下：① `MeshService` 的 `peerEntries`/`routeEntries` → `ConcurrentHashMap`（消除 CME 根源，迭代弱一致，`iterator().remove()` 兼容）② `startDebugLoop` 循环体包 try-catch（任何快照异常只跳过本轮，不拖垮进程）③ `DebugStats.snapshot()` 对外部注入 provider 全部 `runCatching`（内核不信任外部读取器，单字段失败返回默认值）。v1.1.6 的 FGS 权限补充保留（本身是正确性修复）。

### 已验证内容
- **v1.1.7 启动崩溃真根因修复验证**：`testDebugUnitTest` **117/117 通过，0 失败**（ConcurrentHashMap 替换后 MeshService 路由/心跳/removePeer 测试全回归）；`assembleDebug` **BUILD SUCCESSFUL**（versionCode 69 / versionName 1.1.7，merged manifest 仍含 FGS 权限）；APK `MeshChat-v1.1.7-debug.apk`。⚠️ 待用户真机覆盖安装确认能打开（CME 修复为主，FGS 权限为辅）。
...
### 当前阻塞
- **⚠️ v1.1.3 二轮融合 + v1.1.4 删除对话修复 + v1.1.5 调试中心 + v1.1.6 FGS 权限 + v1.1.7 崩溃真根因（CME）修复，待用户真机验证后提交推送**：改动全部在工作区未提交（v1.1.3 的 19 修改 + 8 新增，v1.1.4 的 6 文件小改，v1.1.5 的 DebugStats 内核/埋点/UI 约 12 文件，v1.1.6 的 Manifest 权限 2 行，v1.1.7 的 ConcurrentHashMap/startDebugLoop 防御 3 文件）。**待办**：① 用户安装 `MeshChat-v1.1.7-debug.apk` 验证（**重点：能正常打开、前台常驻通知出现**；其余：删除对话后 Mesh 页节点消失、安全中心入口、对话归档/删除/未读、附近/历史分组、新图标、**调试中心各板块实时数据 + 7 项调节 + 暂停/清零**）；② 确认后 git add + commit + push origin/main。
...
### 下一步首要任务
0. **v1.1.7 真机验证（当前版本）**：用户安装 `MeshChat-v1.1.7-debug.apk` → ① **首要确认：App 能正常打开、前台常驻通知出现（CME 崩溃修复生效）** ② 删除对话后 Mesh 页该节点消失（在线节点会重新被发现，属预期）③ 安全中心入口/本地评估 ④ 对话归档/删除/未读/最后消息 ⑤ Mesh 页附近/历史分组 + 新图标 ⑥ **调试中心**：进 Profile→调试中心，观察五板块实时数据（收发包速率随心跳/消息跳动、BLE 统计、节点 RSSI、送达确认率）、调节 7 项（刷新间隔/窗口/单位/板块/排序/暂停/清零）生效 → 确认后提交推送 origin/main（v1.1.3 融合 + v1.1.4 修复 + v1.1.5 调试中心 + v1.1.6 FGS 权限 + v1.1.7 CME 修复）。
...
- **v1.1.7 启动崩溃真根因（CME）修复**：`mesh/service/MeshService.kt`（`peerEntries`/`routeEntries` 第 164/156 行 `LinkedHashMap` → `ConcurrentHashMap` + import——**本次崩溃根因文件**）、`ui/MeshChatViewModel.kt`（`startDebugLoop` 循环体 try-catch 防协程崩溃拖垮进程）、`mesh/debug/DebugStats.kt`（`snapshot()` 对外部 provider 全部 runCatching）、`app/build.gradle.kts`（v1.1.7/69）、`AI_CONTEXT.md`
