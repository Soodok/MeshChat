...
### 当前阻塞
- **⚠️ v1.1.1 崩溃已修复并推送（22ef6b9）**：v1.1.1 覆盖升级启动闪退（Room 迁移校验失败，根因见进度区）→ v1.1.2/64 修复（补 Entities.kt 索引声明），**用户真机覆盖安装确认正常、旧数据保留**。已提交推送 origin/main（`22ef6b9`，v1.1.1 融合 + v1.1.2 修复合为一个 commit）。推送时顺带 rebase 了队友远程提交 `c724b7a`（删 .trae）/`532bca6`（删 贡献.md），无冲突。
...
### 下一步首要任务
0. **v1.1.2 已真机确认并推送（22ef6b9）**，继续 v1.1.0 多跳中继真机验收。
