...
- **当前版本：v1.0.25（versionCode 61，构建时间 2026-08-04 18:45）**——版本更新规则：每次构建后 bump，安装包命名 `MeshChat-vX.Y.Z-debug.apk` 存于工程根目录
...
- `gradlew assembleDebug`：**BUILD SUCCESSFUL**，APK `MeshChat-v1.0.24-debug.apk`（19,192,426 B）。
...
- **v1.0.25 节点/连线再缩小**（用户反馈"框大小正常了，但节点显示大小仍然比较大"）：节点半径 16/13/11 → **10/8/7**（相对框占比 4.4%→2.8%），连线宽 mesh 1.5→1.2、SEARCHING 3→2.2、DIRECT 3.5→2.6、REACHABLE 2.5→1.8，描边 4→3、短 ID 字号 16→11、昵称 20→15、三角缩小，物理 springLen 80→60、repulsion 3500→2600（更紧凑）。框保持全宽正方形不动。测试 64/64 通过，APK 19,192,426 B。

### 下一步首要任务
0. **v1.0.25 真机验证（当前版本）**：安装 `MeshChat-v1.0.25-debug.apk`，重点看① **节点/连线更小**（相对框占比 2.8%）② 图更紧凑、附近节点显示更多 ③ 拖拽/中心引力正常 ④ 蓝牙关→开自动恢复（v1.0.24）⑤ 聊天列表不抖动、消息收发/文件传输回归。
