...
- **当前版本：v1.1.2（versionCode 64，构建时间 2026-08-04 22:56）**——版本更新规则：每次构建后 bump，安装包命名 `MeshChat-vX.Y.Z-debug.apk` 存于工程根目录
...
### 当前阻塞
- **⚠️ v1.1.1 崩溃已修复（v1.1.2），待用户真机确认 + 提交推送**：用户安装 v1.1.1 打开即闪退（Room 迁移校验失败，根因见进度区），已修复为 v1.1.2/64（补 Entities.kt 索引声明）。**待办**：① 用户真机覆盖安装 `MeshChat-v1.1.2-debug.apk` 确认不再卡退（覆盖安装即可，无需清数据）；② 确认后 git add + commit + push origin/main（改动含 v1.1.1 融合 15 文件 + v1.1.2 修复，均未提交）。
...
### 下一步首要任务
0. **v1.1.2 真机确认（当前版本）**：用户覆盖安装 `MeshChat-v1.1.2-debug.apk`（无需清数据，直接覆盖 v1.1.1）→ 确认打开不再闪退、旧数据（消息/会话/节点）完整保留 → 确认后提交推送 origin/main（v1.1.1 融合 15 文件 + v1.1.2 修复，均未提交）。
