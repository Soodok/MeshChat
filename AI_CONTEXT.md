...
- **当前版本：v1.0.24（versionCode 60，构建时间 2026-08-04 09:28）**——版本更新规则：每次构建后 bump，安装包命名 `MeshChat-vX.Y.Z-debug.apk` 存于工程根目录
...
### 下一步首要任务
0. **v1.0.24 真机验证（当前版本）**：安装 `MeshChat-v1.0.24-debug.apk`，重点复现**蓝牙关→开**：正常对话时一台关蓝牙 1-2s 再开——① 该设备应**自动恢复**（无需重进/点重新发现），能重新搜到对端、收到对端补发的消息（消息状态从"正在推送"自动收敛为已送达）② 对端应自动重新连上该设备 ③ 对照验证关蓝牙后 3-4s 帧到达延迟属发送端系统行为（关蓝牙设备断开 GATT 有延迟，非接收端 bug）。其余回归：拓扑图节点大小（v1.0.23）、聊天列表不抖动、蓝牙重搜、消息收发/文件传输。
